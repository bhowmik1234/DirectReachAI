...
import { useEmailContext } from '@/components/providers/context/Employee';


const MultiInputForm = () => {
  const { data: session } = useSession();
  const { cemail, cjobTitle, cjobRole } = useEmailContext();
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if(generatedEmail){
        const resp = await axios.post("/api/less-credit", { email: session?.user?.email, name: session?.user?.name, credit: 1 });
